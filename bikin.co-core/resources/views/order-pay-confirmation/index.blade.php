            ...
            <div class="uk-overflow-auto">
                <table class="uk-table uk-table-divider" id="ts-issues">
                    ...
                    <tr role="row "class="odd">
                        ...
                        <a href="#modal-upload-bukti-transfer{{ $data[$i]->id }}" data-uk-toggle class="sc-button sc-button-mini sc-button-success">Unggah</a>
                        ...
                        <td>
                            <a href="#modal-track-order{{$data[$i]->id}}" onclick="trackOrder({{ $data[$i]->id }})" data-uk-toggle class="sc-button sc-button-mini" href="#">TRACK</a>
                            <div style="margin-top: 10px;">
                                {{--                                <a href="{{ url('invoice/print/'.$data[$i]->id) }}" target="_blank" class="sc-button sc-button-mini">Invoice</a>--}}
                                <a href="{{ route('so.printable.inv.dp', $data[$i]->id) }}?action=export"
                                   target="_blank" class="sc-button sc-button-mini">Invoice</a>
                            </div>
                        </td>
                        ...
                        @else
                            ...
                            @elseif($data[$i]->status == 3)
                                <span class="uk-label uk-label-primary">MENUNGGU KONFIRMASI</span>
...
<div id="modal-upload-bukti-transfer{{ $data[$i]->id }}" data-uk-modal>
    <div class="uk-modal-dialog">
        <button class="uk-modal-close-default" type="button" data-uk-close></button>
        <div class="uk-modal-body" data-uk-overflow-auto>
            <p>Unggah bukti transfer dari pelanggan</p>
            <form action="{{ route('order_payment.add') }}" method="post" id="form" enctype="multipart/form-data">
            {{csrf_field()}}
                <input type="file" name="upload" class="upload" value="">
                <input type="hidden" name="id_order_payment" class="id_order_payment" value="{{ $data[$i]->id }}">
                <input type="hidden" name="status" value="{{ $data[$i]->status }}">
                <input type="hidden" name="type" class="type" value="{{ $data[$i]->payment_type }}">
                <input type="hidden" name="payment_total" class="payment_total" value="{{ $data[$i]->part_paid_amount }}">
                <input type="hidden" name="proof_payment" class="proof_payment" value="{{ $data[$i]->proof_payment }}">
                <div style="margin-top: 35px;">
                    <input type="submit" class="sc-button sc-button-primary sc-js-button-wave-light" href="#" disabled value="Unggah Bukti Transfer">
                </div>
...
<!-- new -->
@for ($i = 0; $i < $count ; $i++)
<div id="modal-view-image{{$data[$i]->id}}" class="uk-flex-top" data-uk-modal>
    <div class="uk-modal-dialog uk-width-auto uk-margin-auto-vertical">
        <button class="uk-modal-close-outside" type="button" data-uk-close></button>

        @if($data[$i]->orderPayment->isEmpty())
        @else
        <img style="max-height: 600px;" src="{{ $data[$i]->proof_payment }}" alt="">
        <div style="margin: 15px;">
            <p>Diunggah pada {{ $data[$i]->log_created_at }} Pukul {{ $data[$i]->log_created_time }} </p>
            <!-- 31-12-2021 Pukul 13:43 -->
        </div>
...
@push('scripts')
 ...
 function trackOrder(id)
 {
    ...
    //     $.ajax({
    //         type: "GET",
    //         url: '{!! route('track_order.getMaterial', [':id']) !!}'.replace(':id',id),
    //         success: function (data) {
    //             $('.reference_design').empty();
