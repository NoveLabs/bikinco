...
@foreach ($dataStep0 as $value)
  <div class="uk-modal" id="step-{{ $value->id }}">
        <div class="uk-modal-dialog">
            ...
            <form class="uk-form-stacked" action="{{ route('order_proses.vendor.step0') }}" method="post" enctype="multipart/form-data">
            @csrf
                ...
                <div class="uk-modal-footer uk-text-right">
                    ...
                    <button type="submit"  class="md-btn md-btn-success md-btn-wave-light">Mulai Dikerjakan</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="uk-modal" id="step-{{ $value->id }}">
        <div class="uk-modal-dialog">
            <div class="uk-modal-header">
                ...
                <div class="uk-margin-medium-bottom">
                    <div class="uk-width-medium-1-1">
                        <div class="md-card-content">
                            ...
                            <form enctype="multipart/form-data" method="post" action="{{ route('order_proses.vendor.upload') }}" class="dropzone" id="dropzone">	
                            @csrf
                            <input type="hidden" name="id" value="{{ $value->id }}">
                            </form>
                        </div>
                ...
                <div class="uk-modal-footer uk-text-right">
                    <button type="button" class="md-btn md-btn-flat uk-modal-close md-btn-wave">Tutup</button>
                    <button data-uk-modal="{target:'#modal_catatan'}" type="button" class="md-btn md-btn-success md-btn-wave-light">Ajukan Penilaian QC</button>
                </div>
        </div>
    </div>
@endforeach
...
@foreach ($dataStep1 as $value)
    ...
    <div class="uk-modal" id="modal_catatan">
        <div class="uk-modal-dialog">
            <button type="button" class="uk-modal-close uk-close"></button>
            <p class="uk-text-bold">Berikan catatan.</p>
            <form enctype="multipart/form-data" method="post" action="{{ route('order_proses.vendor.catatan') }}">	
            @csrf
                <div class="uk-grid">
                    <div class="uk-width-1-1">
                        <div class="uk-form-row">
                            <!-- <label>Proses telah dikerjakan</label> -->
                            <textarea cols="30" rows="4" class="md-input" name="notes" placeholder="Proses telah dikerjakan"></textarea>
                            <input type="hidden" name="id" value="{{ $value->id }}">
                        </div>
                    </div>
                </div>
...
Dropzone.options.dropzone =
         {
            ...
            removedfile: function(file) 
            {
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    url: '{{ route("order_proses.vendor.deleteUpload") }}',
                    data: {filename: name},
	...
	{	
		$.ajax({
            type: "GET",
            url: '{!! route('order_proses.vendor_image.all', [':id']) !!}'.replace(':id', id),
            success: function (data) {
        ...
        $.ajax({
            type: "GET",
            url: '{!! route('order_proses.vendor_note.all', [':id']) !!}'.replace(':id', id),
            success: function (data) {
