                                ...
                                <table class="uk-table uk-table-align-vertical">
                                    ...
                                    @for ($i = 0; $i < $count; $i++)
                                        <tr>
                                            ...
                                            @else
                                             <button data-uk-modal="{target:'#modal-bukti-dp'}" onclick="showGambar({{ $data[$i]->id }})" class="md-btn md-btn-mini md-btn-wave-light">BUKTI DP</button>
                                            @endif
                                            ...
                                            <td class="uk-text-nowrap">
                                            <a href="{{ route('vendor.order_proses.progress', $data[$i]->id) }}" target="blank" class="md-btn md-btn-mini md-btn-primary md-btn-wave-light">PROGRESS</a>
                                            </td>
                                        </tr>
                                    @endfor
...
<script>
    function showGambar(id)
    {
        $.ajax({
            type: 'GET',
            url: '{{ route('getImageVendorDP', [':id']) }}'.replace(':id', id),
            success: (data) => {
               $('#bukti_payment').empty();
               var html = '<img style="max-height:600px;"  src="' + data.proof_payment_dp +'" alt="" ';
