    ...
    <div id="sc-page-top-bar" class="sc-top-bar uk-flex-middle">
        <div class="sc-top-bar-content sc-padding-medium-ends uk-flex-1">
            <div class="uk-flex uk-flex-column uk-flex-1">
                <h1 class="sc-top-bar-title uk-text-uppercase uk-margin-small-bottom">Proses Order</h1>
          ...
          <tbody>
                @for ($i = 0; $i < $count ; $i++)
                    <tr role="row "class="odd">
                        <td>
                            <a href="{{ route('order_item_step.progress', $data[$i]->id) }}" data-uk-toggle class="sc-button sc-button-mini sc-button-warning">PROGRESS</a>
                        </td>
                        <td>
                            <a href="#modal-track-order{{ $data[$i]->id }}" onclick="trackOrder({{ $data[$i]->id }})" data-uk-toggle class="sc-button sc-button-mini">TRACK</a>
                            <br>
                            <a href="{{ route('so.printable.spk.customer', $data[$i]->id) }}?action=export"
                               target="blank" class="sc-button sc-button-mini">SPK CUSTOMER</a>
                            <br>
                            <a href="{{ route('so.printable.spk.vendor', $data[$i]->id) }}?action=export" target="blank"
                               class="sc-button sc-button-mini">SPK VENDOR</a>
                            <br>
                            <a href="{{ url('pd_upload/print/'.$data[$i]->id) }}" target="_blank" class="sc-button sc-button-mini">PRINT ARTWORK</a>
                            <br>
                            <a href="{{ route('vendor.printable.billOfMaterial', $data[$i]->id) }}?action=export"
                        ...
                        <td>
                        @if($data[$i]->status == 3)
                        <span class="uk-label uk-label-warning">Belum Selesai</span>
                        @else
                        <span class="uk-label uk-label-info">Belum dikerjakan</span>
                        @endif
                        </td>
                    </tr>
                    @endfor
