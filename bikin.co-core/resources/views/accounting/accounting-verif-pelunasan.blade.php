                        ...
                        <td>
                            ...
                            <div style="margin-top: 10px;">
                                <a href="{{ route('so.printable.inv.repayment', $data[$i]->id) }}?action=export"
                                   target="blank" class="sc-button sc-button-mini">Invoice</a>
                            </div>
                        </td>
                        <td><a href="{{ route('so.printable.quotation', $data[$i]->id) }}?action=export" target="blank">BQ-{{ $data[$i]->id }}</a>
                        </td>
...
<div id="modal-view-image{{$data[$i]->id}}" data-uk-modal>
    <div class="uk-modal-dialog">
        ...
        <div class="uk-modal-footer uk-text-right">
            <a href="{{ route('so.printable.inv.repayment', $data[$i]->id) }}?action=export" target="blank"
            ...
            <form  enctype="multipart/form-data" role="form" id="add_pelunasan" method="POST" action="">
                @csrf
                <input type="hidden" name="status" value="1">
                <input type="hidden" name="type" value="{{$data[$i]->type}}">
                <input type="hidden" name="id_order_payment" value="{{$data[$i]->id_order_payment}}">
                <input type="hidden" name="order_id" value="{{$data[$i]->order_id}}">
                <input type="hidden" name="proof_payment" value="{{$data[$i]->proof_payment_name}}">
                <input type="hidden" name="payment_total" value="{{$data[$i]->payment_total}}">
                <input type="hidden" name="id" value="{{$data[$i]->id}}">
            <button type="submit" class="sc-button sc-button-success" href="#">
...
<div id="modal-reject-form{{$data[$i]->id}}" data-uk-modal>
    <div class="uk-modal-dialog">
        ...
        <form  enctype="multipart/form-data" role="form" method="POST" action="{{ route('accounting_pelunasan.tolak')}}">
        @csrf
...
=======
                                    ...
                                    <ul class="uk-list uk-margin">
>>>>>>> 05c9b9b48c00939d31fd68b2573acc31820aeb86
...
@push('scripts')
 ...
 function trackOrder(id)
 {
    ...
    $(document).ready(function () {
        ...
        $('#add_pelunasan').submit(function(event){
             ...
             UIkit.modal.confirm('Konfirmasi Bukti Pembayaran Ini?').then(function(){
                ...
                type: 'POST',
                url: "{{ route('accounting_pelunasan.add')}}",
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
                success: (data) => {
                    ...
                    setTimeout(function() {
                        window.location.href = "{{ route('accounting_pelunasan-index') }}";
                    }, 5);
                },
                error: function(data) {
                    UIkit.modal.alert(data.responseJSON.message).then(function () {
