...
<div id="sc-page-wrapper">
	<div id="sc-page-content">
        <div class="uk-card uk-margin-top">
            <h3 class="uk-card-title">Dalam Komplain Ke Vendor</h3>
                    ...
                    <table class="uk-table uk-table-striped uk-table-hover uk-table-middle">
                        ...
                        @for ($i = 0; $i < $count; $i++)
                            <tr>
                                ...
                                <td>
                                @if($data[$i]->complainVendor[0]->status == 1)
                                <span class="uk-label uk-label-warning">Menunggu Respon Vendor</span>
                                @elseif($data[$i]->complainVendor[0]->status == 2)
                                <span class="uk-label uk-label-success">Sudah Dibalas Vendor</span>
                                @endif
                                </td>
                                <td><a href="#modal-lihat{{ $data[$i]->id }}" data-uk-toggle onclick="getDataImage({{ $data[$i]->orderItems[0]->id }})" class="mdi mdi-file-outline sc-icon-square"></a></td>
                            </tr>
                            <?php $number++;
                            ?>
                        @endfor
                    ...
                    <div class="uk-card uk-margin">
                        ...
                        <div class="uk-card-body">
                            <div class="uk-position-relative uk-visible-toggle uk-light" data-uk-slider="center: true">
                                <ul class="uk-slider-items uk-grid" id="img_carousel{{$data[$i]->orderItems[0]->id}}">
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (i = 0; i < data.data.length; i++) {
                        ...
                        var html = '<li class="uk-width-3-4">';
                         html += '<img src="{{ URL::asset("/") }}' + data.data[i].image +'" alt="">';
                         html += '</li>';
            ...
            success: function (data) {
                ...
                for (i = 0; i < data.data[0].complain_vendor.length; i++) {
                        ...
                        // data.data[0].complain_vendor[i].created_at;

                        var complain = '<li class="uk-open">';
                         complain += '<h3 class="uk-accordion-title md-bg-light-blue-50">Pengirim:'+ stts + ' - '+ data.data[0].complain_vendor[i].created_at +'</h3>';
                         // Sabtu, 6 Mar 2021 12:45
                         complain += ' <div class="uk-accordion-content">';
                         complain += '<p style="margin-bottom: 20px;">Jenis Komplain : ' + compl + '</p>';
                         complain += ''+ data.data[0].complain_vendor[i].notes +'';
                         complain += '<br><br>';
                         complain += '<a style="margin-top: 20px;" href="'+data.data[0].complain_vendor[i].attachment + '" target="_blank">>> Lihat Lampiran</a>';
                         complain += '</div>';
                         complain += '</li>';
