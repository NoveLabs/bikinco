    ...
    <div id="sc-page-content">
        <div class="uk-alert-icon" data-uk-alert>
            ...
            <div class="uk-flex uk-flex-middle">
                ...
                <div class="uk-alert-content">
                    <h5>Pada halaman ini terdapat data master klaster konsumen dari Bikin-co</h5>
                    <p>Anda dapat menambah, mengedit dan menghapus data klaster konsumen sesuai kebutuhan Anda.</p>
                </div>
            </div>
        </div>
        ...
        <div class="uk-card uk-margin">
            <div class="uk-flex-middle sc-padding sc-padding-medium-ends uk-grid-small" data-uk-grid>
                <div class="uk-flex-1">
                    <h3 class="uk-card-title">Data Master Klaster</h3>
                </div>
            ...
            <div class="uk-card-body">
                <table id="customer-works-table" class="customer-works-table uk-table uk-table-striped dt-responsive">
                    <thead>
                        <tr>
                            <th>No</th>
                            <th>Nama</th>
...
<div id="add-modal" data-uk-modal>
    <div class="uk-modal-dialog">
        ...
        <div class="uk-modal-header">
            <h2 class="uk-modal-title">Input Data Klaster</h2>
        </div>
        <div class="uk-modal-body">
            <div class="uk-margin">
                <label>Nama Klaster<sup>*</sup></label>
    ...
    <div class="uk-modal-dialog">
        ...
        <div class="uk-modal-footer uk-text-right">
            <form action="" style="width: 100px;">
                <a id="sc-js-modal-confirm" style="margin-right: 66%;" class="delete-customer-works uk-text-left sc-button sc-button-danger sc-js-button-wave-light" href="#">Hapus</a>
            </form>
            <button class="sc-button sc-button-flat sc-button-flat-danger uk-modal-close" type="button">Batal</button>
...
<script>
    function showDetail(id)
    {
        ...
        $.ajax({
            type: "GET",
            url: '/customer-works/' + id,
            success: function (data) {
                //Set up data for edit modal
                $("input[name=edit-customer-works-id]").val(data.data.id);
                $("input[name=name]").val(data.data.name);

                //Set up credentials for edit and delete action...
                $(".delete-customer-works").attr('data-id', id);
            },
            error: function (data) {
	...
	$(document).ready(function () {
        ...
        var table = $('#customer-works-table').DataTable({
            processing: true,
            serverSide: true,
            retrieve: true,
            pageLength: 10,
            ajax: "{{ route('customer-works') }}",
            columns: [
        ...
        //Delete action
        $('.delete-customer-works').on('click', function (e) {
			UIkit.modal.confirm('Apakah anda yakin akan menghapus data ini ?').then(function () {
                e.preventDefault();

                var params = {
                    _token: '{{ csrf_token() }}',
                    id: $(".delete-customer-works").attr('data-id'),
                }

                $.ajax({
                    type: "DELETE",
                    url: "{{ route('customer-works.delete') }}",
                    data: params,
                    async: false,
                    success: function (data) {
        ...
        $('#edit_form').submit(function(event) {
            ...
            var id = $("input[name=edit-customer-works-id]").val();
