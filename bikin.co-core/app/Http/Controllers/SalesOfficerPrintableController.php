...
class SalesOfficerPrintableController extends Controller
{
    # Private Variables
    private $artwork;
    private $category;
    ...
    # SO - Quotation function
    public function quotation(Request $request, $id)
    {
        $compact = $this->getData($request, $id);

        if ($request->action == 'export') {

            return PDF::loadView('document-exports.exports.so-quotation-export', $compact)->setPaper('a4')
            ->download('Quotation-BP-' . $id . '.pdf');
    ...
    # SO - SPK Customer
    public function spkCustomer(Request $request, $id)
    {
        $compact = $this->getData($request, $id);

        if ($request->action == 'export') {

            return PDF::loadView('document-exports.exports.so-spk-customer-export', $compact)->setPaper('a4')
                    ->download('SPK-Customer-BP-' . $id . '.pdf');
    ...
    # SO - SPK Vendor
    public function spkVendor(Request $request, $id)
    {
        $compact = $this->getData($request, $id);

        if ($request->action == 'export') {

            return PDF::loadView('document-exports.exports.so-spk-vendor-export', $compact)->setPaper('a4')
                    ->download('SPK-Vendor-BP-' . $id . '.pdf');
    ...
    # SO - Down Payment Invoice
    public function downPaymentInvoice(Request $request, $id)
    {
        $compact = $this->getData($request, $id);

        if ($request->action == 'export') {

            return PDF::loadView('document-exports.exports.so-invoice-downpayment-export', $compact)->setPaper('a4')
                    ->download('Invoice-Pembayaran-Order-BP-' . $id . '.pdf');
    ...
    # SO - Down Payment Invoice - Paid
    public function downPaymentInvoicePaid(Request $request, $id)
    {
        $compact = $this->getData($request, $id);

        if ($request->action == 'export') {

            return PDF::loadView('document-exports.exports.so-invoice-downPayment-paid-export',
                    $compact)->setPaper('a4')
                    ->download('[PAID]-Invoice-Pembayaran-Order-BP-' . $id . '.pdf');
    ...
    # SO - Repayment Invoice
    public function repaymentInvoice(Request $request, $id)
    {
        $compact = $this->getData($request, $id);

        if ($request->action == 'export') {

            return PDF::loadView('document-exports.exports.so-invoice-repayment-export', $compact)->setPaper('a4')
                    ->download('Invoice-Pelunasan-Order-BP-' . $id . '.pdf');
    ...
    # SO - Repayment Invoice - Paid
    public function repaymentInvoicePaid(Request $request, $id)
    {
        $compact = $this->getData($request, $id);
        ...
        if ($request->action == 'export') {
            ...
            return PDF::loadView('document-exports.exports.so-invoice-repayment-paid-export',
                    $compact)->setPaper('a4')
                    ->download('[PAID]-Invoice-Pelunasan-Order-BP-' . $id . '.pdf');
    ...
    {
        $data = $this->order->getDataInvoice($id);

        $hasil = $data->orderItems[0]->hasProduct->price * $data->total_item;

        return PDF::loadView('document-exports.exports.order-payments.invoice', compact('data', 'hasil'))->setPaper('a4')
                    ->download('Order-Payments-' . $data->id . '.pdf');
    }
}
